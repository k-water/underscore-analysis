...
// 最外层为立即执行函数(IIFE)
// 使用立即函数可以创建一个独立的沙箱似的作用域，它是函数的一种特殊调用方式，
// 利用了JavaScript的函数作用域的概念。
// 这样可以防止其他代码对该函数内部造成影响，而且不会创造全局变量防止污染全局空间。
(function() {
    ...
    // 将this赋值给root
    // 在客户端root的值为`window` 在服务端(node)中为`exports`
    var root = this;
    ...
    // are declared here.
    // ES5原生方法 倘若浏览器支持 underscore优先使用原生方法
    var
    ...
    var optimizeCb = function(func, context, argCount) {
      // context === void 0 => 没有this指向 void 0返回undefined
    ...
    var createAssigner = function(keysFunc, undefinedOnly) {
      return function(obj) {
        ...
        for (var index = 1; index < length; index++) {
          ...
          for (var i = 0; i < l; i++) {
            var key = keys[i];
             // 如果不是undefinedOnly模式，则会覆盖原来key的value
             // 如果是undefinedOnly模式，则只会设置原来为undefined的属性
    ...
    // Aliased as `any`.
    /**
     * 迭代对象的所有元素 如果任意一个元素通过真值检测 返回true
     * @param obj
     * @param predicate
     * @param context
     * @example
    ...
    /**
     * 对于一个对象集合 返回满足where条件的对象
     * @param obj 待迭代参数
     * @param attr where条件对象
     */
